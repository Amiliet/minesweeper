...
void mouseCallback(ObjectID object, int x, int y, MouseAction action) {
	...
	if (object == startButton) {
		...
		endGame();
			...
			if (b == -1) {
				showMessage("실배!");
				startButton = createObject("restart.png", room, 600, 400, true);
				endButton = createObject("end.png", room, 600, 350, true);
			}
			else if (b == 1) {
				showMessage("성공!");
				startButton = createObject("restart.png", room, 600, 400, true);
...
int main(void) {
	...
	startButton = createObject("start.png", room, 600, 400, true);
	endButton = createObject("end.png", room, 600, 350, true);
	...
	for (int i = 0; i < ROW; i++) {
		for (int k = 0; k < COL; k++) {
			picture[i * 10 + k] = createObject("11.png", room, 350+k * 50, 100+i * 50, false);
			scaleObject(picture[i * 10 + k], 0.5f);
		}
	}
